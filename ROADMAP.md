...
- [ ] **Flaky property tests** (S) — `__tests__/properties/transparency-dashboard-*` and `content-hierarchy` use fast-check's default 100 runs with random seeds (against the repo's `numRuns: 3` convention) → intermittent full-suite failures that pass in isolation. Cap `numRuns` / fix seeds for deterministic CI
